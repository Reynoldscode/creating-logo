...
class Color {
  constructor(color) {
    if (typeof color === 'string') {
      ...
      throw new Error('Invalid color format. Color must be provided as a string.');
    }
  }
...
// Already in hexadecimal format
    if (rgb.startsWith('#') && (rgb.length === 4 || rgb.length === 7)) {
      ...
      throw new Error('Invalid color format. Color must be provided as a hexadecimal string, RGB string, or color name.');
    }
  }
